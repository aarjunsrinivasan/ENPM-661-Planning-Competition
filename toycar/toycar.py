...
def boundary_check(i,j):
    if (i<tot or j>=50-tot or j<tot or i>=50-tot):
        return 0
    else:
...
def  convergence(a,goal_node):
            if((np.square(a[0]-goal_node[0]))+ (np.square(a[1]-goal_node[1])) <=np.square(0.4)) :
                            return 0
            else:
                return 1
...
velPub = rospy.Publisher('/toycar/joint3_position_controller/command', Float64, queue_size=100)
phi1Pub = rospy.Publisher('/toycar/joint1_position_controller/command', Float64, queue_size=100)#Right
phi2Pub = rospy.Publisher('/toycar/joint2_position_controller/command', Float64, queue_size=100)#Left
